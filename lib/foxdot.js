...
export default {
	...
	config: {
		...
		samplesFolder: {
			...
			description: 'Leave empty to use the default samples folder.',
			type: 'string',
			default: '',
			order: 2,
		},
	...
	start() {
		...
		const samplesFolder = atom.config.get('foxdot.samplesFolder');
		let command = ['-m', 'FoxDot', '--pipe'];
		if (samplesFolder !== "") {
			atom.notifications.addInfo('Using samples from ' + samplesFolder)
			command = command.concat(['-d', samplesFolder]);
		}
